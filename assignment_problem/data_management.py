...
def get_availability(data, days_list, hours_list, teachers_list):
    ...
    data['Value'] = 1
    sub = data[['Teacher','To','Day','Value']]
    sub.rename(columns ={'To':'From'},inplace=True)

    avail = pd.concat([data,sub])
    avail['From'] = avail['From'].astype('str').str.split(':').apply(lambda x:x[0])
    avail = aux.merge(avail,left_on = ['Teacher', 'Day','Hour'],right_on=['Teacher', 'Day','From'],how='left')
